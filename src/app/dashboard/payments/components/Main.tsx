...
// const Main = () => {
//   // --- State ---
//   // Initialize as empty array [] to prevent "undefined" errors on first render
...
//     setLoading(true);
//     try {
//       // 1. Fetch Stats (Only once or on mount)
//       if (!stats) {
//         const statsData = await paymentHistoryClient.getStats();
//         setStats(statsData);
//       }
...
//         params.investment__selected_option__project__investment_type = sectorFilter;
//       }

//       const data = await paymentHistoryClient.getTransactions(params);

//       // --- FIX: Robust Data Handling ---
//       // This checks if 'results' exists (DRF Pagination) or if 'data' itself is the array.
//       let safeResults: Transaction[] = [];
//       let safeCount = 0;

//       if (data && Array.isArray(data.results)) {
//         // Scenario A: Standard DRF Pagination ({ count: 10, results: [...] })
//         safeResults = data.results;
//         safeCount = data.count;
//       } else if (Array.isArray(data)) {
//         // Scenario B: No Pagination (Backend returned raw list [...])
//         // We cast it to unknown first to satisfy TS if types conflict
//         safeResults = data as unknown as Transaction[];
//         safeCount = safeResults.length;
//       }

//       setTransactions(safeResults);
//       setTotalCount(safeCount);

//       // Calculate pages safely (prevent division by zero)
//       const pageSize = 10;
//       setTotalPages(safeCount > 0 ? Math.ceil(safeCount / pageSize) : 1);
...
//     // Handle potential null/undefined from backend
...
//             />
//             <input
//               className="w-full pl-10 pr-4 py-2 bg-[#f8f7f6] border-none rounded-lg text-sm focus:ring-1 focus:ring-[#d0a539] outline-none transition-all"
//               placeholder="Search reference, project or location (Press Enter)"
//               type="text"
//               value={searchQuery}
...
//               </select>
//               <Calendar
//                 className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-[#d0a539]"
//                 size={16}
//               />
//             </div>
...
//             <div className="relative">
//               <select
//                 className="appearance-none bg-[#f8f7f6] border-none rounded-lg text-xs font-bold uppercase tracking-widest pl-4 pr-10 py-2.5 focus:ring-1 focus:ring-[#d0a539] cursor-pointer outline-none text-[#171512]"
//                 value={sectorFilter}
//                 onChange={(e) => {
...
//                   // Empty State - Safe check for length
//                   <tr>
//                     <td
//                       colSpan={6}
//                       className="px-6 py-10 text-center text-[#171512]/50 font-medium"
//                     >
...
//                           <span
//                             className={`h-2 w-2 rounded-full ${getSectorColor(txn.investment_type)}`}
//                           ></span>
...
//                           <span className="text-[10px] text-[#171512]/40 ml-4">
//                             Installment #{txn.installment_number}
//                           </span>
//                         )}
//                       </td>
...
//                         <button className="text-[#d0a539] text-[10px] font-black uppercase tracking-widest hover:underline decoration-2 underline-offset-4 opacity-0 group-hover:opacity-100 transition-opacity">
//                           Receipt
//                         </button>
//                       </td>
//                     </tr>
...
const Main = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (sectorFilter && sectorFilter !== "All") {
        params.investment__selected_option__project__investment_type =
          sectorFilter;
      }
      ...
      setTransactions(safeResults);
      setTotalCount(safeCount);
      setTotalPages(safeCount > 0 ? Math.ceil(safeCount / 10) : 1);
    } catch (error) {
      console.error("Failed to fetch transactions:", error);
      setTransactions([]);
    } finally {
      ...
      <div className="grid grid-cols-2 gap-4 text-xs border-t border-[#171512]/5 pt-4 mb-4">
        ...
        <div className="col-span-2 flex items-center gap-2 text-[#171512]/60 bg-[#f8f7f6] p-2 rounded-lg">
          ...
          <span className="font-mono text-[10px] tracking-wider truncate w-full">
            Payment Reference Number: {txn.payment_reference || "NO-REF"}
          </span>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-[#f8f7f6] min-h-screen text-[#171512] font-sans">
      <main className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-8 pt-24 lg:pt-10">
        {/* Page Header & Stats */}
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-8 lg:mb-10">
