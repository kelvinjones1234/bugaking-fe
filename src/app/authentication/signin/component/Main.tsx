...
import { useAuth } from "@/context/AuthContext";
import Link from "next/link";
import Image from "next/image"; // Optimization 1: Next.js Image
import { useRouter, useSearchParams } from "next/navigation";
import { apiClient } from "@/app/offers/api/projectApi";
import { Loader2 } from "lucide-react";
...
// ==========================================
// Optimization 2: React.memo prevents this heavy section from re-rendering
...
const HeroSection = memo(() => {
  return (
    <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden h-full">
      ...
      {/* Optimization 1: Using Next.js Image for LCP & Bandwidth efficiency */}
      <div className="absolute inset-0 z-0">
        <Image
          src="/signin_img.jpg"
          alt="Agriculture Investment Background"
          fill
          priority // Prioritizes loading this image (LCP fix)
          sizes="50vw" // Tells browser this takes up half the screen width
...
const SignInForm = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Only clear error if one exists to avoid unnecessary state updates
    if (error) clearError();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      // Optimization 3: Check localStorage only on success, not render
      const pendingData = localStorage.getItem("pending_investment");
