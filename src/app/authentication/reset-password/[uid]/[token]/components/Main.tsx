...
import { passwordResetClient } from "@/app/dashboard/api/passwordResetApi";
...
const ResetPasswordForm = () => {
  const params = useParams(); // Retrieves [uid] and [token] from URL
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await passwordResetClient.confirmReset(
      ...
      setStatus("error");
      setErrorMessage(
        err.response?.data?.error || "Invalid link or server error.",
      );
    } finally {
      setLoading(false);
    }
